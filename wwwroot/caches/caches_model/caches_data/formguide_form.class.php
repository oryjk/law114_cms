...
class formguide_form {
	...
	function text($field, $value, $fieldinfo) {
		...
		return '<input type="text" name="info['.$field.']" id="'.$field.'" size="'.$size.'" value="'.$value.'" '.$this->no_allowed.' class="input-text" '.$formattribute.' '.$css.' data-role="none">';
	}
	function textarea($field, $value, $fieldinfo) {
		...
		return "<textarea name='info[{$field}]' id='$field' style='width:{$width}%;height:{$height}px;' $formattribute $css $this->no_allowed data-role=\"none\">{$value}</textarea>";
	}
	...
	function images($field, $value, $fieldinfo) {
		...
		return "<input type='text' name='info[$field]' id='$field' value='$value' size='$size' $this->no_allowed class='input-text' {$formattribute} {$css} data-role=\"none\">";
	}
