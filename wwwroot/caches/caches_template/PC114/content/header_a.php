...
<body onload="myfunction()">
    ...
    <div id="wrapper" class="main clearfix">
        <div class="c_nav" style="background:url(<?php echo $CATEGORYS[$catid]['image'];?>) no-repeat left">
            <ul class="clearfix">
            <?php $i=1?>
            <?php $n=1;if(is_array(subcat($catid))) foreach(subcat($catid) AS $c) { ?>
            <?php if($c['ismenu']==1) { ?>
            <li <?php if($i==1) { ?>class="nocut"<?php } ?>><a href="<?php echo $c['url'];?>"><?php echo $c['catname'];?></a></li>
            <?php } ?>
            <?php $i++?>
            <?php $n++;}unset($n); ?>
            </ul>
