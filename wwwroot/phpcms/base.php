...
//$site_style=$sitelist[1]['template'];
$template_style=$sitelist[1]['template_style'];
$template_array=explode(',',$template_style);
$agent = strtolower($_SERVER['HTTP_USER_AGENT']);
if(strpos($agent, 'iphone') || strpos($agent, 'ipad') || strpos($agent, 'android')){
	$site_style=$template_array[1];
}
else{
	$site_style=$template_array[0];
}
//print($site_style);
//exit();
/* 载入站点参数 */
...
//当前站点模板
define('SITE_TPL',$default_style);

define('SKIN_PATH',str_replace('\\','/',WEB_PATH.'phpcms'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$site_style.DIRECTORY_SEPARATOR.'skin/'));
