...
final class template_cache {
	...
	public function template_parse($str) {
		//pw 2014/1/16
		$str = preg_replace("/\{_CFG\[([a-zA-Z0-9_\']+)\]\}/ie", 'pc_base::$_CFG[$1]', $str);
	...
	public static function pc_tag($op, $data, $html) {
		...
		preg_match_all("/([a-z_]+)\=[\"]?([^\"]+)[\"]?/i", stripslashes($data), $matches, PREG_SET_ORDER);
		...
		//pw 2013/7/3解析字符串变量
		$datas=preg_replace('/\$(\w+)/i',"\\$"."\\1",$datas);
		$str = '';
		...
		if (is_array($data)) {
			...
			foreach ($data as $key=>$val) {
				if (is_array($val)) {
					...
					if (strpos($val, '$')===0) {
						$str .= "'$key'=>$val,";
					//pw 2013/7/3添加，解析字符串中的变量，若要与系统原有操作通用，需修改上面$datas['limit']='$offset,$pagesize'
