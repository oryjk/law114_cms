...
function template($module = 'content', $template = 'index', $style = '') {
	...
	//2013/8/29 pw只以站点设置里面选择的模板为准
	$sitelist = getcache('sitelist','commons');
	$style = $sitelist['1']['default_style'];	
...
//	if(!$style) $style = 'default';
	...
	$templateFile=PC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 3) {
	...
	$multipage = '第 '.$curr_page.' 页&nbsp;&nbsp;&nbsp;'.'共 1 页 '.$num.' '.L('page_item');
	if($num > $perpage) {
		$page = $setpages+1;
		...
		$multipage = '第 '.$curr_page.' 页&nbsp;&nbsp;&nbsp;'.'共 '.$pages.' 页 '.$num.' '.'条&nbsp;&nbsp;&nbsp;';
		if($curr_page>0) {
			$multipage .= ' <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$curr_page-1)).'" class="a1">'.L('previous').'</a>';
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>1)).'">1</a>..';
			} else {
				$multipage .= ' <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>1)).'">1</a>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$i)).'">'.$i.'</a>';
			} else {
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$pages)).'">'.$pages.'</a> <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$curr_page+1)).'" class="a1">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$pages)).'">'.$pages.'</a> <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$curr_page+1)).'" class="a1">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span>'.$pages.'</span> <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$curr_page)).'" class="a1">'.L('next').'</a>';
		} else {
			$multipage .= ' <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$pages)).'">'.$pages.'</a> <a href="'.pw_get_url('',array('q','page'),array('q'=>$_GET[q],'page'=>$curr_page+1)).'" class="a1">'.L('next').'</a>';
		}
	}
...
function pw_get_url($page='',$del_para=array(),$add_para=array()){
	if(empty($page))$page=$_SERVER['SCRIPT_NAME'];
	if(!is_array($del_para))$del_para=array($del_para);
	if(!is_array($add_para))$add_para=array($add_para);
	parse_str($_SERVER['QUERY_STRING'],$para);
	foreach($del_para as $v){
		unset($para[$v]);
	}
	$para=array_merge($para,$add_para);
	$para_str=http_build_query($para);
	return $page.(empty($para_str)?'':'?'.$para_str);
}
...
function catpos($catid, $symbol=' > '){
	...
	foreach($arrparentid as $catid) {
		...
		$pos .= $symbol.' <a href="'.$url.'" title="'.$category_arr[$catid]['catname'].'">'.$category_arr[$catid]['catname'].'</a> ';
	}
	return $pos;
}
 ...
 * @param $keyword      关键词
 */
function seo($siteid, $catid = '0', $title = '', $description = '', $keyword = '') {
