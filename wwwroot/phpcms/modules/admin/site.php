...
class site extends admin {
	...
	public function edit() {
		...
		if ($data = $this->db->get_one(array('siteid'=>$siteid))) {
			$map_info =unserialize($data['map_info']);
			$kefu_conf=unserialize($data['kefu_conf']);
			if($map_info['center_map']){
				$center_map=explode(',',$map_info['center_map']);
				$mark_map  =explode(',',$map_info['mark_map']);
			}
			if(empty($kefu_conf)){
				$kefu_conf=array('kefu_switch'=>'1','kefu_style'=>'style1','kefu_pos'=>'right','kefu_show_way'=>'hover','top_away'=>'140');
			}
			...
			if (isset($_POST['dosubmit'])) {
				...
				$msg_no   =$_POST['msg_no'];
				$zixun_url=$_POST['zixun_url'];
				$map_info =$_POST['map_info'];
				$kefu_conf=$_POST['kefu_conf'];
				...
				$setting = trim(array2string($_POST['setting']));
				$sql = array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template, 'setting'=>$setting, 'default_style'=>$default_style,'phone_no'=>$phone_no,'msg_no'=>$msg_no,'zixun_url'=>$zixun_url,'map_info'=>addslashes(serialize($map_info)),'kefu_conf'=>addslashes(serialize($kefu_conf)));
