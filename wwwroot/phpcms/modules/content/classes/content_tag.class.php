...
class content_tag {
	...
	public function count($data) {
		...
		if($data['action'] == 'lists') {
			...
			//pw 2013/7/3 自定义标签分页
		}elseif($data['action'] == 'getRecord'){
			$data=str_replace(array('eq'),array('='),$data);
			
			$this->db->table_name=$this->db->db_tablepre.$data['name'];
			
			if(isset($data['join'])){
				$this->db->table_name.=' '.preg_replace('/(\w+)(?=\s+as\s+\w+(,|\)|\s))/i',"{$this->db->db_tablepre}$1 ",$data['join']);
			}
			return $this->db->count($data['where']);
		}
	}
	//pw 2013/7/1添加
	function getRecord($data){
		...
		$sql='select '.$data['fields'].' from '.$this->db->db_tablepre.$data['name'];
		if(isset($data['join'])){
			$sql.=' '.preg_replace('/(\w+)(?=\s+as\s+\w+(,|\)|\s))/i',"{$this->db->db_tablepre}$1 ",$data['join']);
		}
		if(isset($data['where'])){
			//pw 2013/9/27添加
			$sql.=' where '.preg_replace('/(\w+)(?=\s+as\s+\w+(,|\)|\s))/i',"{$this->db->db_tablepre}$1 ",$data['where']);
	...
	public function category($data) {
		...
		//2013/8/9 pw添加排序
		if(is_array($categorys) && $data['order']){
			$ordert=explode(' ',$data['order']);
			$categorys=array_sort($categorys,$ordert[0],$ordert[1]?$ordert[1]:'asc');
		}
