...
class index {
	...
	function __construct() {
		...
		$siteid = 1;
		$r=$this->site_db->get_one(array('siteid'=>$siteid));
		$template_array=explode(',',$r['template_style']);
		$agent = strtolower($_SERVER['HTTP_USER_AGENT']);
		if(strpos($agent, 'iphone') || strpos($agent, 'ipad') || strpos($agent, 'android')){
  			$default_style=$template_array[1];
 		}
		else{
			$default_style=$template_array[0];
		}
		$sql = array('default_style'=>$default_style,'template'=>$default_style);
		if ($this->site_db->update($sql, array('siteid'=>$siteid))) {
	...
	public function kefu() {
		...
		if($kefu_conf['td_code']){
			$kefu_conf['td_image']='http://s.jiathis.com/qrcode.php?url='.urlencode($kefu_conf['td_code']);	
		}
		include template('content','kefu',$default_style);
	}
	//内容页
	...
	function share(){
		...
		include template('content','share');
	}
	//pw 2013/7/5 新增地图查看功能
	function map(){
		//SEO
		$SEO = seo(SITEID);
		$siteinfo=siteinfo(SITEID);
		$map_info=unserialize($siteinfo['map_info']);
		if($map_info['center_map']){
			$center_map=explode(',',$map_info['center_map']);
			$mark_map  =explode(',',$map_info['mark_map']);
		}

		include template('content','map');
	}
