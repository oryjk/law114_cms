...
class index {
	...
	public function show() {
		...
		if (isset($_POST['dosubmit'])) {
            session_start();
            if($_SESSION['formguide_code']==strtolower($_POST['info']['l_yzm'])){
                unset($_SESSION['formguide_code']);
                session_commit();
            }else{
                showmessage(L('form_error_yzm'), HTTP_REFERER);
                session_commit();
                return;
            }
			...
			if (($setting['allowmultisubmit']==0 && $re['datetime']) || ((SYS_TIME-$re['datetime'])<$this->M['interval']*60)) {
				$_GET['action'] ? exit : showmessage(L('had_participate'), APP_PATH.'index.php');
			}
