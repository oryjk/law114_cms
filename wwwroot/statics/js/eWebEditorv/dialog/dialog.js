...
try{
	myBrowser = dialogArguments.myBrowser;
		...
		try{myBrowser = opener.myBrowser;
		}catch(e){}
}
...
function InitObjUpform(){
	if (!ObjUpform)
	{
		try{ObjUpform = document.myuploadform;}
		catch(e){}
	}else{
//	ObjUpform.btn_upfile.style.left = (objform.uploadfile.offsetWidth - objform.btn_upfile.offsetWidth - 2) + "px";
//	ObjUpform.originalfile.style.width = (objform.uploadfile.offsetWidth - objform.btn_upfile.offsetWidth - 2) + "px";
	objform.uploadfile.filters.alpha.opacity = 0;
	}
}

function startupload() {
	//saupfile组件，显示进度条"1"==config.UploadObject || "2"==config.UploadObject || "3"==config.UploadObject
    if (config.ServerExt == "asp"){
        document.getElementById("divProcessing").innerHTML = "<iframe name=prog width=240 height=70 src='../asp/progress.asp?style=" + config.StyleName + "' border=0 frameborder=0 scrolling='no' onload=\"document.all('prog').style.height=prog.document.body.scrollHeight;\"></iframe>"
	    document.getElementById("divProcessing").style.left = (tabDialogSize.offsetWidth - document.getElementById("divProcessing").offsetWidth) / 2;
	    document.getElementById("divProcessing").style.top = (tabDialogSize.offsetHeight - document.getElementById("divProcessing").offsetHeight) / 2;
	}else{
...
function BaseTrim(str){
	...
	for(var i=0;i<str.length;i++){
		...
		if ((act=="all" || act=="left") && thelStr==" "){lIdx++;}
		if ((act=="all" || act=="right") && therStr==" "){rIdx--;}
	}
		str=str.slice(lIdx,rIdx);return str;
}
...
function SelectColor(what){
	...
	if (arr) {dEL.value=arr;sEL.style.backgroundColor=arr;}
}
...
function imgButtonOver(el){
	if(!el["imageinitliazed"]){
		el["oncontextmenu"]= new Function("event.returnValue=true") ;
...
function getUploadForm(s_Type){
	...
	switch(s_Type){
		case "image":
		    s_MaxSize = config.AllowImageSize;break;
		case "flash":
		    s_MaxSize = config.AllowFlashSize;break;
		case "media":
		    s_MaxSize = config.AllowMediaSize;break;
		case "file":
			s_MaxSize = config.AllowFileSize;break;
		default:
			return "";
	}
	var n_MaxSize = parseFloat(s_MaxSize)*1024;
	//var html = "<iframe name='myuploadformtarget' style='display:none;position:absolute;width:0px;height:0px' src='blank.htm'></iframe><iframe name='myuploadform' src='upform.htm?ext=" + config.ServerExt + "&type="+s_Type+"&style="+config.StyleName+"&cusdir="+config.CusDir+"&maxsize="+n_MaxSize+"&skin=" + config.Skin + "&lang=" + lang.ActiveLanguage + "' style='margin:0px;padding:0px;width:100%;height:20px;border:0px' border=0 frameborder=0 scrolling='no'></iframe>";
	var html = "<iframe name='myuploadformtarget' style='display:none;position:absolute;width:0px;height:0px' src='blank.htm'></iframe>"+"<form action='..//" + config.ServerExt + "/upload." + config.ServerExt + "?action=save&type="+s_Type+"&style="+config.StyleName+"&cusdir="+config.CusDir+"' method=post name=myuploadform enctype='multipart/form-data' style='margin:0px;padding:0px;width:100%;border:0px' target='myuploadformtarget'>"+"<input type=file name='uploadfile' size=1 style='width:100%' onchange=\"this.form.originalfile.value=this.value;try{ActionAdd(this.form,this);doPreview();} catch(e){}\">"+"<input type=hidden name='MAX_FILE_SIZE' value='"+n_MaxSize+"'>"+"<input type=hidden name='originalfile' value=''>"+"<input name='temp_action' type=hidden value=''></form>";
	return html;
}

function ActionAdd(o,t){
   if (t.value!="")
   {   o.originalfile.value = t.value;
	   if (o.temp_action.value=="")
	   {
		  o.temp_action.value = o.action;
	   }
	   o.action = o.temp_action.value + "&" + t.name + "=" + escape(t.value);
   }
   //alert(o.action);
}
function getUploadErrDesc(s_Flag, s_Ext, s_Size){
	...
	switch(s_Flag){
		case "ext":
		    s_ErrDesc = lang["ErrUploadInvalidExt"] + ":" + s_Ext;break;
		case "size":
			s_ErrDesc = lang["ErrUploadSizeLimit"] + ":" + s_Size + "KB";break;
		//new add by weiwei
		case "size_ext":
			s_ErrDesc = lang["ErrUploadInvalidExt"] + ":" + s_Ext + "\n" + lang["ErrUploadSizeLimit"] + ":" + s_Size + "KB";break;
		case "file":
			//s_ErrDesc = lang["ErrUploadInvalidFile"];break;
			s_ErrDesc = lang["ErrUploadInvalidExt"] + ":" + s_Ext;break;
		case "style":
		    s_ErrDesc = lang["ErrInvalidStyle"];break;
		//new add by weiwei
		case "noneup":
		    s_ErrDesc = lang["ErrAllUpLimit"];break;
		default: 
			s_ErrDesc = s_Flag;
	}
	return s_ErrDesc;
